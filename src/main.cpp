...
// void editRating(std::vector<Media>& mediaList, int index){
//     int newRating = promptIntUntilValid("New Rating: ", isValidRating);
//     mediaList[index].rating = newRating;
// }

// void editStatus(std::vector<Media>& mediaList, int index){
//     std::string prompt = "Enter Status(";
//         for(int i = 0; i < (int)STATUS_TABLE.size(); i++){
//             prompt += STATUS_TABLE[i].label;
//             if(i != (int)STATUS_TABLE.size() - 1)
...
//         mediaList[index].completionStatus = stringToStatus(newStatus);
// }
// void editAmountViewed(std::vector<Media>& mediaList, int index){
//     int newAmountViewed = promptIntUntilValid("New Amount Viewed: ", isValidAmountViewed);
//     mediaList[index].amountViewed = newAmountViewed;
// }
// void editViewdate(std::vector<Media>& mediaList, int index){
//     std::string newDate = shortcutDate(promptStringUntilValid("Enter Date(YYYY-MM-DD)(c = today): ", isValidDate));
//     mediaList[index].dateLastViewed = newDate;
// }
// void editName(std::vector<Media>& mediaList, int index){
//     std::string newName = promptName();
//     mediaList[index].name = newName;
// }
// void editSource(std::vector<Media>& mediaList, int index){
//     std::string newSource = promptStringUntilValid("Enter Source: ", isValidSource);
//     mediaList[index].source = newSource;
// }

// void editMediaV2(std::vector<Media>& mediaList, EditField field){
...
//     std::vector<EditStruct> EditStructs = {
//         {EditField::RATING,   [&]() { editRating(mediaList, targetIndex); }},
//         {EditField::STATUS,   [&]() { editStatus(mediaList, targetIndex); }},
//         {EditField::AMOUNT,   [&]() { editAmountViewed(mediaList, targetIndex); }},
//         {EditField::VIEWDATE, [&]() { editViewdate(mediaList, targetIndex); }},
//         {EditField::NAME,     [&]() { editName(mediaList, targetIndex); }},
//         {EditField::SOURCE,   [&]() { editSource(mediaList, targetIndex); }},
//     };

//     for(const auto& edit : EditStructs){
//         if(field == edit.field){
//             edit.editFunction();
//             break;
//         }
